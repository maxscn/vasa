...
| Public package surface    | Early. Root package direction says `@opeinspection/skriva` should be the tree-shakeable public package.        | Export a stable React-targeted API centered on `<SkrivaEditor editor={editor} enrichments={...} />`, first-party enrichments, layout, style, diagnostics, and native renderers. | API snapshot tests or type tests for intended exports only.                                            |
...
- Decide which APIs are public under `@opeinspection/skriva` and which packages remain internal.
...
- Consolidate the public API through `@opeinspection/skriva`.
